            ...
            <div class="col-12">
                ...
                <div class="card mb-4 mx-4"
                                ...
                                <div class="mt-2">
                                    @if ($contest->status === 'draft')
                                        <form action="{{ route('contests.publish', $contest) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" style="background-color:#56C596;" class="btn text-white btn-sm mb-0">Publish</button>
                                        </form>
                                    @elseif ($contest->status === 'scheduled' && ! $contest->hasStarted())
                                        <form action="{{ route('contests.unpublish', $contest) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-outline-secondary btn-sm mb-0">Back to draft</button>
                                        </form>
                                    @endif
                                    <a href="{{ route('contests.leaderboard', $contest) }}" class="btn btn-outline-dark btn-sm mb-0">Leaderboard</a>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body pt-3">
                        <div class="row">
                            ...
                            <div class="col-lg-6 mb-4">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    ...
                                    <span class="badge bg-gradient-dark" x-text="paper.length + ' question' + (paper.length === 1 ? '' : 's')"></span>
                                </div>
                                    ...
                                    <template x-for="(q, i) in paper" :key="q.question_id">
                                        <div class="d-flex align-items-start justify-content-between border-bottom py-2 px-2">
                                            <div class="pe-2">
                                                ...
                                                <span class="badge badge-sm bg-gradient-secondary" x-text="q.subject || 'No subject'"></span>
                                                <template x-if="q.difficulty">
                                                    <span class="badge badge-sm bg-gradient-info" x-text="q.difficulty"></span>
                                                </template>
                                            </div>
                                            <button type="button" class="btn btn-link p-0 text-red-500" x-show="!locked"
                                ...
                                <div class="border-radius-md" style="max-height: 29rem; overflow-y: auto;">
                                    <template x-for="q in available" :key="q.question_id">
                                        <div class="d-flex align-items-start justify-content-between border-bottom py-2 px-2">
                                            ...
                                            <button type="button" class="btn btn-link p-0 text-success" x-show="!locked"
                        ...
                        <template x-if="error">
                            <div class="alert alert-danger text-white text-xs mt-3 mb-0" x-text="error"></div>
                        </template>
                    </div>
                </div>
            </div>
