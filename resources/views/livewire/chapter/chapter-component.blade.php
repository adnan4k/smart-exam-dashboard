            ...
            <div class="card border border-slate-200/80 rounded-2xl bg-white shadow-sm overflow-hidden mb-4">
                ...
                <div class="card-header border-b border-slate-100 p-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <h5 class="font-bold text-slate-800 text-base mb-0.5 tracking-tight">Chapters & Modules</h5>
                            <p class="text-xs text-slate-400 mb-0">Manage study chapters, curriculum units, and media assets.</p>
                        </div>
                        <button
                            @click="$dispatch('chapterModal')"
                            class="btn-brand self-start sm:self-auto shadow-sm"
                            type="button">
                            <i class="fa-solid fa-plus text-xs"></i> New Chapter
                        </button>
                    </div>
                </div>
                        ...
                        <table class="table align-items-center mb-0 w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th>Chapter Name</th>
                                    <th>Description</th>
                                    <th class="text-center w-24">Media</th>
                                    <th class="text-center w-24">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($chapters as $num => $chapter)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p class="text-xs font-bold text-slate-800 mb-0">{{ $chapter->name }}</p>
                                                    <p class="text-[11px] text-slate-400 mb-0">ID: #{{ $chapter->id }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            @if($chapter->image)
                                                <img src="{{ asset('storage/'.$chapter->image) }}"
                                                     alt="{{ $chapter->name }}"
                                                     class="w-8 h-8 object-cover rounded-lg border border-slate-200 shadow-xs mx-auto cursor-pointer hover:scale-110 transition-transform duration-200"
                                    ...
                                    <tr>
                                        <td colspan="5" class="p-0">
                                            <x-empty-state
                                                ...
                                                description="Get started by creating your first curriculum chapter."
    ...
    @if($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto"
            ...
            <div class="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 text-center animate-fade-in">
                ...
                <h5 class="text-base font-bold text-slate-800 mb-1">Delete Chapter</h5>
                <p class="text-xs text-slate-500 mb-4">
                    Are you sure you want to delete <span class="font-bold text-slate-700">"{{ $chapterToDelete->name ?? '' }}"</span>? This will permanently delete this chapter from the syllabus.
                </p>

                @if($chapterToDelete)
                    <div class="bg-slate-50 rounded-xl p-3 border border-slate-100 mb-5 text-left text-xs text-slate-600">
                        <span class="text-slate-400">Description:</span>
                        <p class="font-medium text-slate-700 mt-1 mb-0">{{ $chapterToDelete->description ?? 'No description' }}</p>
                    </div>
                @endif
